...
- `/iris-clear-threads` - in the current channel, lists every thread whose `owner_id` is the bot (active + archived public/private) via Discord REST and deletes each one. No database state is read or written. Admin only.
